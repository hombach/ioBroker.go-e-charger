...
class go_e_charger extends utils.Adapter {
    ...
    async ParseStatusAPIV1(status, iWB) {
        ...
        void this.projectUtils.checkAndSetValueNumber(`${basePath}.statistics.TEST1RFID${Number(status.rca)}.chargedEnergy`, Number(status.eca) / 10, `Charged energy for RFID chip ${Number(status.rca)}`, "kWh", "value.energy.consumed");
        // WiP 802
