...
class go_e_charger extends utils.Adapter {
	...
	private stateMachineTimeout: ioBroker.Timeout | undefined;
	...
	private async onReady(): Promise<void> {
		...
		try {
			for (const [iWB, wallBox] of wallBoxList.entries()) {
				...
				// resolve the per-wallbox current limits from the installation limit and the optional
				// per-box user overrides (hardware caps are folded in later once the charger is read)
				...
				if (wallBox.minAmp && wallBox.maxAmp && wallBox.minAmp > wallBox.maxAmp) {
					this.log.warn(
						`Wallbox ${iWB}: configured minAmp (${wallBox.minAmp} A) exceeds maxAmp (${wallBox.maxAmp} A); using ${limits.minCurrent}/${limits.maxCurrent} A`,
					);
				}
	...
	/*****************************************************************************************/
	private scheduleStateMachine(): void {
	...
	async StateMachine(): Promise<void> {
		...
		try {
			...
			this.log.error(`Unhandled error in state machine cycle: ${(error as Error).message}`);
		} finally {
			this.scheduleStateMachine();
		}
	} // END StateMachine
	...
	async Charge_Config(Allow: string, Ampere: number, LogMessage: string, iWB: number): Promise<void> {
		...
		const commands = buildChargerCommands(Allow === "1", Ampere, this.wallboxInfoList[iWB].Firmware);
		if (!commands) {
		...
		for (const command of commands) {
			try {
				...
				this.log.warn(
					`Error: ${String(error)} by writing to charger ${iWB}: ${this.config.wallBoxList[iWB].ipAddress} ${command.parameter}=${command.value}`,
				);
				this.log.error(`Please verify IP address of charger ${iWB}: ${this.config.wallBoxList[iWB].ipAddress} !!!`);
				return;
			}
	...
	private async planChargeManagerCycle(): Promise<(WallboxChargePlan | undefined)[]> {
		...
		served.forEach((iWB, index) => {
			...
			// surplus-sharing control loop: one concise line per served charger, in priority order
			const decision = decisions[index];
			const carState = this.wallboxInfoList[iWB].CarState;
			this.log.debug(
				`ChargeManager charger ${iWB}: surplus ${Math.round(decision.availablePower)} W → optimal ${decision.optimalCurrent ?? 0} A, ramp ${decision.nextState.currentAmp} A, action=${decision.action} (${decision.reason}), car=${carState}`,
			);
	...
	private planTotalCurrentBudget(chargePlans: (WallboxChargePlan | undefined)[]): TotalCurrentAllocation[] {
		...
		// total-current control loop: one concise line for the whole fleet (requested -> granted per box)
		let used = 0;
